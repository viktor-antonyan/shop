...
import {deleteAllPizzas} from "../../redux/actions/actionCreators";
import emptyCartImg from "../../img/empty-cart.png"
import {Link} from "react-router-dom";

export const Cart = () => {
    const dispatch = useDispatch()
    const {totalPrice, totalCount, cartPizzas} = useSelector(({cart}) => cart)
    const pizzaKeys = Object.keys(cartPizzas)
    const pizzasToCart = pizzaKeys.map(key => cartPizzas[key].items[0])

    const deletePizzasToCart = () => {
        window.confirm('delete pizzas ?') && dispatch(deleteAllPizzas())
    }
    const myPizzasInCart = () => {
                ...
                {totalCount !== 0 ? <div className="cart">
                        <div className="cart__top">
                            ...
                            <div className="cart__clear">
                                ...
                                <span onClick={deletePizzasToCart}>Очистить корзину</span>
                            </div>
                        </div>
                        <div className="content__items">
                            {pizzasToCart.map(pizza => <PizzaCart key={pizza.id} pizza={pizza}
                        ...
                        <div className="cart__bottom">
                            ...
                            <div className="cart__bottom-buttons">
                                ...
                                <button onClick={myPizzasInCart} className="button pay-btn">
